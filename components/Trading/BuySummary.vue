<template>
        ...
        <div>
            ...
            <div class="divide-y">
                ...
                <ListItem :title="$t('purchase_amount')">{{ $n((orderPreview?.amount - orderPreview?.fee) / 100) }} {{
                    currency
                }}</ListItem>
                <ListItem :title="$t('transaction_fee')">{{ $n(orderPreview?.fee / 100) }} {{ currency }}</ListItem>
                <ListItem :title="$t('totalAmount')">{{ $n(orderPreview?.amount / 100) }} {{ currency }}</ListItem>
            </div>
...
const props = defineProps({
    ...
    PaymentAccount:{
        type: Object as PropType<PaymentAccount>
    },
    title:{
        type:String
    },
...
const execute = async ()=>{
    ...
    if(props.PaymentMethod == PaymentMethods.bankAccount){
        request.payment_account_id = props.PaymentAccount?.id;
    }
    try{
...
const payment = computed(()=>{
    ...
    if(props.PaymentMethod == PaymentMethods.bankTranfer){
        ...
        if(props.PaymentAccount){
            let iban = '';
            let bankName = ''
            props.PaymentAccount.payment_account_specs.forEach(spec=>{
