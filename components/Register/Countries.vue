    ...
    <Listbox as="div" v-model="selected">
      ...
      <div class="relative mt-1" v-if="!isLoading && !error">
        <ListboxButton class="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
          <span class="block truncate">{{ selected.name_translation_key }}</span>
          ...
          <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ListboxOption as="template" v-for="country in countries" :key="country.id" :value="country" v-slot="{ active, selected }">
              <li :class="[active ? 'text-white bg-indigo-600' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-3 pr-9']">
                <span :class="[selected ? 'font-semibold' : 'font-normal', 'block truncate']">{{ country.name_translation_key }}</span>
  ...
  isLoading.value = true;
    countries.value  = await CountryService.getCountryList(props.all);
     ...
     if(!props.selectedCountry)
     {
        ...
        emit("change",countries.value[0].id);
     }
     else{
